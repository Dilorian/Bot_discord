...
from bot.services import achievement_service, quest_service, economy_service
...
async def _build_profile_embed(guild: discord.Guild, member: discord.Member) -> discord.Embed:
    ...
    async with async_session_factory() as session:
        balance = await economy_service.get_balance(session, guild.id, member.id)
...
class ProfileView(discord.ui.View):
    ...
    async def inventory(self, interaction: discord.Interaction, button: discord.ui.Button):
        async with async_session_factory() as session:
            items = await economy_service.list_inventory(session, self.guild.id, self.member.id)
        desc = "\n".join(f"• **{i.item_name}** × {i.quantity}" for i in items) or "Инвентарь пуст."
        await interaction.response.send_message(
            embed=info_embed(f"🎒 Инвентарь — {self.member.display_name}", desc), ephemeral=True
        )

...
from bot.services.economy_service import credit, add_inventory
        ...
        async with async_session_factory() as session:
            ...
            if quest.reward_money:
                await credit(
                    ...
                    transaction_type="quest_reward", actor_discord_id=self.discord_id,
                    description=f"Награда за задание: {quest.title}", reference=f"quest:{quest.id}",
                )
            if quest.reward_case:
                await add_inventory(
                    session, self.guild_id, self.discord_id,
                    item_key=f"case:{quest.reward_case}", name=f"Кейс: {quest.reward_case}",
                    item_type="case", quantity=1, item_metadata={"case_key": quest.reward_case},
                )

# Family Bot — Этапы 1–4

Discord-бот для семьи STIFFLER (Majestic RP). Реализованы **Этап 1** (база),
**Этап 2** (профили, XP, уровни, репутация) и **Этап 3** (задания, достижения,
рейтинги, сезоны и Family Pass) из ТЗ.

## Что уже реализовано

### Этап 4 — Экономика
- Личные балансы и уникальные транзакции с остатком после операции.
- `/balance`, `/pay`, `/transfer`, `/daily`, `/weekly`.
- Безопасные переводы с блокировкой строк БД для защиты от двойного списания.
- Магазин: `/shop`, `/buy`, административное `/economy shop_add`.
- Инвентарь: `/inventory`, предметы и титулы как хранимые сущности.
- Кейсы: создание, настройка наград и весов, открытие, остатки и редкости.
- Семейный банк: `/bank`, `/bank_deposit`, `/bank_withdraw`.
- Административные `/economy give|take` и аудит операций.
- Награды заданий и Family Pass теперь могут зачислять деньги/XP и выдавать предметы/кейсы.
...
### Экономика — основные команды

| Команда | Описание | Требуемое право |
|---|---|---|
| `/balance` | Баланс участника | — |
| `/pay` / `/transfer` | Перевод денег | — |
| `/daily` / `/weekly` | Бонусы | — |
| `/shop` | Магазин | — |
| `/buy` | Покупка | — |
| `/inventory` | Инвентарь | — |
| `/case` | Открытие кейса | — |
| `/bank` | Баланс семейного банка | — |
| `/bank_deposit` | Взнос в семейный банк | — |
| `/bank_withdraw` | Снятие из семейного банка | `manage_economy` |
| `/economy give` | Выдать деньги | `manage_economy` |
| `/economy take` | Списать деньги | `manage_economy` |
| `/economy shop_add` | Создать товар | `manage_economy` |
| `/economy case_add` | Создать кейс | `manage_economy` |
| `/economy case_reward` | Добавить награду кейса | `manage_economy` |
| `/economy_history` | История транзакций | — |
...
- **Этап 4** — экономика, магазин, кейсы, инвентарь, семейный банк — реализован в текущей версии;
